local setting = {
	centerRoom = {x = 33443, y = 31545, z = 13}, -- centro sala
	storage = Storage.GraveDanger.Tombs.KingZelosTimer,
	bossPosition = {x = 33443, y = 31545, z = 13}, -- posicao boss
	kickPosition = {x = 32172, y = 31918, z = 8}, -- pra onde toma kick
	playerTeleport = {x = 33437, y = 31572, z = 13} -- pra onde player vai
}
...
function vlarkorthLever.onUse(creature, item, fromPosition, target, toPosition, isHotkey)
	...
	local clearVlarkorthRoom = Game.getSpectators(Position(setting.centerRoom), false, false, 28, 28, 28, 28)       
	...
	for i = 0, 4 do
		...
		for i, player in ipairs(players) do
            ...
            player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'You have 30 minute(s) to defeat the King Zelos.')
				addEvent(function()
					local spectatorsVlarkorth = Game.getSpectators(Position(setting.centerRoom), false, false, 28, 28, 28, 28)
